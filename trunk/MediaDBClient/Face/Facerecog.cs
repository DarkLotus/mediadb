...
namespace MediaDBClient.Face
{
    public class Facialrecogg
    {
        ...
        public Image<Gray,Byte> DetectFaceInImage(Image<Gray,Byte> ImgToScan)
        {
            ...
            MCvAvgComp[][] faces = ImgToScan.DetectHaarCascade(haarcasc,1.1,4,Emgu.CV.CvEnum.HAAR_DETECTION_TYPE.DO_CANNY_PRUNING,new Size(20,20));
        ...
        private void LoadArrays()
        {
           // int dbsize = MediaDBwpf.Database.People.SqlitePeople.Getfacedb(out KnownFacearray,out KnowFaceLabelarray);
            Tempload(out KnownFacearray, out KnowFaceLabelarray);
        }

        private void Tempload(out Image<Gray, byte>[] KnownFacearray, out string[] KnowFaceLabelarray)
        {
